      ...
      <table>
        <tr>
          ...
          <td className="b" align="left" rowSpan={2} style={{backgroundColor: "Gold"}}>
            ...
            <p>1. Every incident should be predicted that it could occur beforehand and documented as a potential known error and published in a Knowledge Article in Knowledge Management with instructions on "how to confirm this is what has gone wrong" and "the specific steps to recover the service as quickly as possible". The knowledge database should be easy to search and find the article you are looking for.</p>
            <p>2. Every incident is automatically detected by observability, and raised to the right team with an accurate and concise explanation of what has gone wrong as soon as it is detected. We dont wait for a cusomer to ring and tell us our service is down and we could even link the incident to the knowledge article that explains what to do.</p>
            <p>3. The team that is assigned the incident is prepared and ready to respond as quickly as possible, with the right tools, access to audit/error logs, and the right system access permissions to assess and identify the technical cause of the incident and the steps to recover the service.</p>
            <p>4. Every subsequent incident occurance after the initial event, including from another impacted service, can be related to the initial incident record so that the blast radius of the event can be recognised and reported on.</p>
            <p>5. The team working on the incident have a simple but effective way to communicate the expected outage duration to the customers impacted by the incident. The team are aware of how an outage can impact the customers that depend on this service.</p>
            <p>6. If this is an instance of a critical outage (P1/P2), or a repeated outage then the incident is classified as a Problem, is linked to an existing Problem record or registered in Problem Management so we can work on making sure the incident never occurs again.</p>
        ...
        <tr>
          <td align="left" style={{backgroundColor: "lightGray"}}>
            ...
            <p>Every Knowledge Article must describe how to recognise the specific problem, so you can recognise that this is the answer, and provide the steps to quickly recover from that problem.</p>          </td>
        </tr>
      </table>
    </div>
  )
